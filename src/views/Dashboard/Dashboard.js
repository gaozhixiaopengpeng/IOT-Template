...
//api
import { configGetGoogleMapConfig } from "api/config";
//utils
import { setMapKey } from "utils/auth";
...
export default function Dashboard() {
  ...
  React.useEffect(() => {
    let status = true;
    //同步配置
    configGetGoogleMapConfig().then(res => {
      if (status) {
        setMapKey(res.body.mapKey);
      }
    });
    return () => {
      status = false;
  ...
  return (
    <div>
      {/* <div className={classes.test}>22222</div> */}
      <GridContainer>
